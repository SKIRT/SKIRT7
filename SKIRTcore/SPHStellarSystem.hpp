...
#ifndef SPHSTELLARSYSTEM_HPP
#define SPHSTELLARSYSTEM_HPP
...
/** The SPHStellarSystem class represents stellar systems defined from a set of SPH star particles,
...
class SPHStellarSystem : public StellarSystem
{
    Q_OBJECT
    Q_CLASSINFO("Title", "a stellar system derived from an SPH output file")
...
public:
    ...
    Q_INVOKABLE SPHStellarSystem();

protected:
    /** This function performs setup for the SPH stellar system. The first step is to load the
...
public:
    /** This function returns the monochromatic luminosity \f$L_\ell\f$ of the stellar system at the
        wavelength index \f$\ell\f$. It just reads the appropriate number from the internally
        stored vector. */
    ...
    /** This function simulates the emission of a monochromatic photon package with a monochromatic
        luminosity \f$L\f$ at wavelength index \f$\ell\f$ from the stellar system. It just randomly
        chooses an SPH particle from the \f$N\f$ possible particles by generating a random number
