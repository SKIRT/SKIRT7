...
/** The StellarSurfaceGeometry class is a subclass of the Geometry class. It represents the
    ...
    The density profile is spherically symmetric and is simply \f[ \rho(r) =
    \frac{1}{4\pi\,r_*^2}\, \delta(r-r_*). \f] The special
    aspect of this geometry is that the radiation is not isotropic. From a given point on the
    spherical surface, the angular distribution of the emission is not uniform, but it is zero
    in the hemisphere below the surface (there is no radiation inwards) and proportional to
    \f$\cos\theta'\f$ in the outward hemisphere, where \f$\theta'\f$ is the angle between the
    outward direction and the normal on the surface. */
...
public:
    ...
    /** This function generates a random direction appropriate for the anisotropic outward radiation
        field at the location \f${\bf{r}}\f$. In a spherical coordinate system with the Z' axis normal
        to the surface at the position \f${\bf{r}}\f$, the probability distribution is given by
        ...
        of the vector \f${\bf{k}}\f$ in the standard coordinate system. The link between the standard
        XYZ coordinate system and the one with the Z' axis determined by the normal on the surface is
        a joint rotation over an angle \f$\varphi\f$ along the Z-axis, followed by a rotation
