...
/** The SpiralStructureGeometry class is a Geometry decorator that adds spiral
    ...
    \rho(R,\phi,z) = \rho_{\text{ax}}(R,z) \left\{ (1-w) + w\, C_N \sin^{2N}
    \left[\frac{m}{2}\left( \frac{\ln (R/R_0)}{\tan p}-(\phi-\phi_0)\right) +
    \frac{\pi}{4} \right]\right\}. \f] Apart from the reference to the original
    ...
    that the total mass equals one, \f[ C_N = \frac{1}{2\pi} \int_0^{2\pi} \sin^{2N}
    \left[ \frac{m\,(\gamma-\phi)}{2} \right] {\text{d}}\phi = \frac{\sqrt{\pi}\,
    \Gamma(N+1)}{\Gamma(N+\tfrac12)}. \f] For \f$N=1\f$ the expression for the
    density reduces to \f[ \rho(R,\phi,z) = \rho_{\text{ax}}(R,z) \left\{ 1 +
    w\, \sin \left[m \left( \frac{\ln (R/R_0)}{\tan p} - (\phi-\phi_0)\right)
    \right]\right\}, \f] as in Misiriotis et al. (2000, A&A, 353, 117). Note that
...
public:
    ...
    /** This function verifies the validity of the different parameters. This setup function also fills
...
public:
    ...
    /** This function generates a random position from the geometry by
        ...
        {\text{d}}{\bf{r}} = \rho({\bf{r}})\, {\text{d}}{\bf{r}}\f$. As the decorator basically just alters
        the azimuthal distribution, we start by generating a random position from the original axisymmetric
        geometry that is being decorated, and we can use the cylindrical radius \f$R\f$ and height
        \f$z\f$.
