...
namespace
{
    ...
    // grain size distribution for PAH (both neutral and ionized)
    double dnda_pah(double a)
    {
        ...
        const double b4 = 0.0; // not used
        const double m4 = 0.0; // not used
        // 50% of the PAH grains are neutral, 50% are ionized
        return 0.5 * dnda(a, A, c0, b0, a1, b1, m1, a2, b2, m2, a3, b3, m3, a4, b4, m4);
    }
}
...
void ZubkoDustMix::setupSelfBefore()
{
    ...
    addpopulations(new DraineGraphiteGrainComposition(this),   amin_gra, amax_gra, &dnda_gra, _Ngra);
    addpopulations(new DraineSilicateGrainComposition(this),   amin_sil, amax_sil, &dnda_sil, _Nsil);
    addpopulations(new DraineNeutralPAHGrainComposition(this), amin_pah, amax_pah, &dnda_pah, _Npah);
    addpopulations(new DraineIonizedPAHGrainComposition(this), amin_pah, amax_pah, &dnda_pah, _Npah);
}
