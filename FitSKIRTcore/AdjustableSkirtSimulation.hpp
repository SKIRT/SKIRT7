...
protected:
    /** This function reads the specified ski file into memory, performs a single simulation using
        the default values provided in the ski file, and outputs an xml and tex version of this
        default ski file. */
...
public:
    ...
    /** This function runs the SKIRT simulation specified by the previously loaded ski file after
        ...
        dictionary arguments (see detailed description below). The prefix string, if present, is
        appended to the filename prefix for all output files of this simulation run.

        The \em conditions dictionary contains a set of key/value pairs controlling inclusion or
        exclusion of certain marked sections in the ski file. The key for each dictionary item is a
        string matching a section label in the ski file, as explained below. The value for each
        dictionary item is boolean \c true (to indicate inclusion of the section) or \c false (to
        indicate exclusion of the section).
        ...
        \verbatim
            ...
            <!--}-->
        \endverbatim
        The section start and end markers must be written exactly as shown, embedded in XML
        comments, with the section label immediately following the left curly bracket and
        immediately preceding a colon. The section label must start with a letter and can contain
        only letters, digits and underscore characters. Nested sections are not allowed. If the
        section label matches one of the keys in the condition dictionary handed to this function,
        and the corresponding value is \c false, this function removes the section from the XML
        file. If the corresponding value is \c true, or if the label does not match a key in the
        dictionary, then the section is preserved (i.e. the default is to leave the section in).
...
private:
    /** This private function performs the specified adjustments on the previously loaded ski
        content as described for the performWith() function, and returns the result. If the
        arguments are missing, all conditions are considered to be "true" (i.e. all conditional
        content is included) and all attributes use the default values as provided in the original
        ski file. */
